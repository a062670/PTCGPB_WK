...
IniRead, discordWebhookURL, %A_ScriptDir%\..\Settings.ini, UserSettings, inspectorWebhookURL, ""
...
AddFriends(renew := false, getFC := false) {
	...
	Loop {
		...
		sleep, 1000
		if(count = 0) {
			sendErrorMsg := "Failed to send requests to: " . sendErrorList.MaxIndex() . " friends:"
			for index, value in sendErrorList {
				sendErrorMsg .= value . ","
			}
			; 去除最後一個逗號
			sendErrorMsg := SubStr(sendErrorMsg, 1, -1)
			;MsgBox, %sendErrorMsg%
			LogToDiscord(sendErrorMsg)
		}
		count++
	}
...
resetWindows(){
	...
	Loop
	{
		try {
			...
			; Move the window
			WinMove, %Title%, , 0, 1074, scaleParam, 537
			break
		}
		catch {
			if (RetryCount > MaxRetries) {
